...
static void gpio_handler(unsigned int irq, struct irq_desc *desc)
{
	...
	if (-1 == bit) {
		/* Modification: Removed by Picsell Dois to get rid of GPIO-IRO error messages.
		printk(KERN_ERR "Unknown cpu.%d gpio phy irq=%d, stat=0x%08x, mask=0x%08x\r\n",
			cpu, phy, stat, mask);
		End-of-Modification: Removed by Picsell Dois to get rid of GPIO-IRO error messages.	*/
