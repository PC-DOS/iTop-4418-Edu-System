...
#define	USB_MASS_DEV_PREFIX		"/dev/sd"
#define	USB_MOUNT_POSITION		"/mnt/usb"

class USBStorageTestWnd: public BaseAppWnd
{
public:
	USBStorageTestWnd();
	~USBStorageTestWnd();
...
private:
	...
	{
		USBStorageTestWnd *pObj = (USBStorageTestWnd*)arg;
		pObj->ThreadProc();
...
USBStorageTestWnd::USBStorageTestWnd()
...
void USBStorageTestWnd::Initialize()
{
	NXDiagButton *btn;
...
int USBStorageTestWnd::EventLoop( int lastResult )
{
	NXDiagButton *btn;
	SDL_Event sdl_event;
	bool bExit = false;
...
void USBStorageTestWnd::UpdateWindow()
{
	...
	snprintf(strStatus, sizeof(strStatus), "USB DISK : %.1fMB/%.1fMB", (double)m_DiskInfo.avail/(1024.*1024.), (double)m_DiskInfo.total/(1024.*1024.));
	DrawButton  ( m_Surface, &btn->rect, m_BtnColor );
	DrawString ( m_Surface, m_Font, strStatus, btn->rect );
	SDL_Flip( m_Surface );
}

int	USBStorageTestWnd::Action( SDL_UserEvent *usrEvent )
{
...
void USBStorageTestWnd::ThreadProc()
{
	bool bFound = false;
	char buf[128];
	...
	//
	{
		...
		if( fd2 )
		{
			...
			{
				if( 0 == strncmp(pEntry->mnt_dir, USB_MOUNT_POSITION, 8 ) )
				{
					bFound = true;
					StorageTestUtil *pDisUtil = new StorageTestUtil(USB_MOUNT_POSITION);
					pDisUtil->GetDiskInfo(m_DiskInfo);
					printf("Device Node : %s, Mount Position = %s\n",buf,USB_MOUNT_POSITION);
	...
	while(m_bThreadRunning)
	{
		if( !bFound )
		{
			...
			if( fd )
			{
				...
				{
					if( 0 == strncmp( USB_MASS_DEV_PREFIX, buf, sizeof(USB_MASS_DEV_PREFIX)-1 ) )
					{
						buf[strlen(buf)-1] = 0;
						if( 0 != mount( buf, USB_MOUNT_POSITION, "vfat", MS_SYNCHRONOUS, NULL ) )
						{
	...
	if( bFound == true )
	{
		umount( USB_MOUNT_POSITION );
	}

}


extern "C" int test_usb( int last_result )
{
	int result;
	USBStorageTestWnd *sdWnd = new USBStorageTestWnd();
	sdWnd->Initialize();
	sdWnd->UpdateWindow();
...
//
static NXDiagPluginInfo USBTestPluginInfo = {
	"USB TEST",
	"USB Diagnostic Application",
	"",
	test_usb,
	0,				//	Disable Auto Test
	0,				//	Start Result
