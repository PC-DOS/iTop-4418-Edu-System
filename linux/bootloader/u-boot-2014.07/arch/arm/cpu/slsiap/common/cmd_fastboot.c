...
static int do_fastboot(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	/* add by cym 20160909 for s5p4418 run android 5.1 */
//#ifdef CONFIG_MACH_S5P4418
	p = getenv("androidversion");
	if(!strcmp(p, "5.1"))
	{
		f_parts_default = FASTBOOT_PARTS_DEFAULT_5_1;
	}
//#endif
	/* end add */
...
#else
#ifdef CONFIG_MACH_S5P4418
		update_from_sd("uboot", "u-boot-iTOP-4418.bin");
#else
		update_from_sd("uboot", "u-boot-iTOP-6818.bin");
#endif
#endif

		update_from_sd("boot", "boot.img");

		update_from_sd("cache", "cache.img");

		update_from_sd("system", "system.img");

		update_from_sd("userdata", "userdata.img");
