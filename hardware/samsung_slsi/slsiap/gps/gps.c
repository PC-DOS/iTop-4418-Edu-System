                          ...
                          Token        speed )
{
    ...
    r->fix.speed    = str2float(tok.p, tok.end)/1.775;//1.852
    return 0;
}
...
//*****************************************************************************
	state->fd= open("/dev/ttyAMA3", O_RDWR | O_NONBLOCK | O_NOCTTY);//这里用的是UART1

	if( state->fd < 0){

            	ALOGE("open port /dev/ttySAC3 ERROR..state->fd=%d\n",state->fd); 
	...
	{
           	D("open port:/dev/ttySAC3 succceed..state->fd=%d\n",state->fd);
	}
	...
	if (state->fd < 0) 
	{
   	...
   	/* if ( pthread_create( &state->thread, NULL, gps_state_thread, state ) != 0 ) {
    	...
    	//***********************************************************************
	D("gps state initialized before");
