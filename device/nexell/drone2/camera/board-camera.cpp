...
NXCameraBoardSensor *get_board_camera_sensor(int id) {
    ...
    if (id == 0) {
	    ...
	    //backSensor = new HM5065(nxp_v4l2_sensor0);
        ...
        if (!frontSensor) {
			//frontSensor = new S5K4ECGX(nxp_v4l2_sensor1);
			frontSensor = new GM7150(nxp_v4l2_sensor1);
            if (!frontSensor)
...
uint32_t get_board_capture_skip_frame(int v4l2_sensorId, int width, int height)
{
    ...
    case nxp_v4l2_sensor0:
        //return 2;
	return 10; //dg change 2 to 10 for better picttures
    case nxp_v4l2_sensor1:
        return 2;
    default:
        return 0;
